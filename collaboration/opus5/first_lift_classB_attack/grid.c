...
/* ---------------- class-B instances ---------------- */
static int VALS[16], NVALS = 0;
static int PROFS[4096][16];
static int NPROF = 0;
static void build_profiles(void)
{
    ...
    NPROF = 0;
    int acc[16];
    /* enumerate counts over VALS with sum of counts = Q and weighted sum = R*N */
    int idx[16]; for (int i = 0; i < NVALS; i++) idx[i] = 0;
    /* simple recursive enumeration via an explicit stack */
    int lvl = 0, colsleft = Q, sumleft = R * N;
    int stackcols[16], stacksum[16];
    while (1) {
        if (lvl == NVALS) {
            if (colsleft == 0 && sumleft == 0 && NPROF < 4096) {
                for (int i = 0; i < NVALS; i++) PROFS[NPROF][i] = acc[i];
                NPROF++;
            }
        } else if (lvl == NVALS - 1) {
            int v = VALS[lvl];
            if (((v == 0) ? (sumleft == 0) : (sumleft % v == 0 && sumleft / v == colsleft)) && NPROF < 4096) {
                for (int i = 0; i < lvl; i++) PROFS[NPROF][i] = acc[i];
                PROFS[NPROF][lvl] = colsleft;
                for (int i = lvl + 1; i < NVALS; i++) PROFS[NPROF][i] = 0;
                NPROF++;
            }
        } else {
            stackcols[lvl] = colsleft; stacksum[lvl] = sumleft;
            acc[lvl] = 0;
            colsleft -= 0; sumleft -= 0;
            lvl++; continue;
        }
        /* backtrack */
        while (lvl > 0) {
            lvl--;
            if (lvl >= NVALS - 1) continue;
            acc[lvl]++;
            if (acc[lvl] <= stackcols[lvl] && acc[lvl] * VALS[lvl] <= stacksum[lvl]) {
                colsleft = stackcols[lvl] - acc[lvl];
                sumleft = stacksum[lvl] - acc[lvl] * VALS[lvl];
                lvl++;
                goto cont;
            }
        }
...
{
    for (int a = 0; a < N; a++) if (pc(forb[a]) != R) return 0;
    for (int c = 0; c < Q; c++) {
        int m = mcount(forb, c);
        if (m > R || m < 0) return 0;
        if (((m - N) & 1)) return 0;              /* |V_c| = N - m must be even */
    }
    return 1;
}
...
{
    if (N > 13) return -1;   /* too expensive; status unknown */
    uint32_t V[Q];
    for (int c = 0; c < Q; c++) {
        uint32_t m = 0;
...
/* minimum slack of the (BC) upper bound over all disjoint X, W;
   a negative value is a proof that no completion exists (Lemma BC).      */
static int bc_slack(const uint32_t *forb)
{
    uint32_t V[Q];
    for (int c = 0; c < Q; c++) {
        uint32_t m = 0;
...
static int move(uint32_t *forb)
{
    for (int att = 0; att < 80; att++) {
        ...
        if (rnd64() % 100 < 35) {           /* profile move: m_c += 2, m_c' -= 2 */
            ...
            if (mcount(forb, c) + 2 > R || mcount(forb, cp) - 2 < 0) continue;
...
int main(int argc, char **argv)
{
    ...
    if (!strcmp(mode, "bcanneal")) {
        /* anneal directly on the (BC) slack: a negative slack is a PROOF of
           infeasibility, so this is a calibration of the search machinery on
           the obstruction family that is known to be non-empty for n<=11.  */
        long restarts = atol(argv[3]);
        long steps = atol(argv[4]);
        int best = 1 << 20; long found = 0;
        uint32_t forb[N], cand[N];
        for (long r = 0; r < restarts; r++) {
            int p = rndint(NPROF);
            if (!gen_instance(forb, p)) { r--; continue; }
            int cur = bc_slack(forb);
            for (long s = 0; s < steps; s++) {
                double T = 2.0 * pow(0.05 / 2.0, (double)s / (double)(steps > 1 ? steps - 1 : 1));
                memcpy(cand, forb, sizeof(forb));
                if (!move(cand)) continue;
                if (!classB_ok(cand)) continue;
                int v = bc_slack(cand);
                if (v <= cur || rnd01() < exp((double)(cur - v) / T)) { memcpy(forb, cand, sizeof(forb)); cur = v; }
                if (cur < best) { best = cur; }
                if (cur < 0) {
                    found++;
                    if (solve(forb, 3000000000L, 1) == 0) emit("BCviolating_UNSAT", forb);
                    else emit("BUG_BCviolating_but_SAT", forb);
                    break;
                }
            }
        }
        printf("bcanneal N=%d R=%d seed=%s best (BC) slack = %d  hits=%ld\n", N, R, argv[2], best, found);
        return 0;
    }
    if (!strcmp(mode, "dump")) {
        long count = atol(argv[3]);
        uint32_t forb[N];
        printf("N=%d R=%d Q=%d NVALS=%d NPROF=%d VALS", N, R, Q, NVALS, NPROF);
        for (int i = 0; i < NVALS; i++) printf(" %d", VALS[i]);
        printf("\n");
        for (int p = 0; p < NPROF; p++) { printf("PROF"); for (int i = 0; i < NVALS; i++) printf(" %d", PROFS[p][i]); printf("\n"); }
        for (long it = 0; it < count; it++) {
            int p = rndint(NPROF);
            if (!gen_instance(forb, p)) { it--; continue; }
            for (int i = 0; i < 300; i++) move(forb);
