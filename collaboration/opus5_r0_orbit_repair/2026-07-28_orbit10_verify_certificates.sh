...
replay_dir=$(mktemp -d /private/tmp/orbit10-fixed-replay.XXXXXX)
...
TMPDIR="$replay_dir" PYTHONPYCACHEPREFIX="$replay_dir/pycache" \
    /usr/bin/python3 \
    "$certificate_dir/orbit10_fixed_pair_verify.py" \
    --results "$certificate_dir/orbit10_fixed_pair_results.jsonl" \
    --cnf-dir "$replay_dir"
