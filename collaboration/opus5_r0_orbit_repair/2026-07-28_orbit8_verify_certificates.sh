...
replay_dir=$(mktemp -d /private/tmp/orbit8-fixed-replay.XXXXXX)
...
TMPDIR="$replay_dir" PYTHONPYCACHEPREFIX="$replay_dir/pycache" \
    /usr/bin/python3 \
    "$certificate_dir/orbit8_fixed_pair_verify.py" \
    --results "$certificate_dir/orbit8_fixed_pair_results.jsonl" \
    --cnf-dir "$replay_dir"
