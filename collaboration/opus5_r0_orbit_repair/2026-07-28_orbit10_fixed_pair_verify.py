#!/usr/bin/env python3
"""Independent semantic audit for the orbit-10 fixed-pair certificates.
...
DEFAULT_RESULTS = HERE / "2026-07-28_orbit10_fixed_pair_results.jsonl"
DEFAULT_CNF_DIR = HERE / "2026-07-28_orbit10_fixed_pair_raw"
...
SEARCH = load_module("orbit10_verify_search", SEARCH_PATH)
CLASSIFIER = load_module("orbit10_verify_classifier", CLASSIFIER_PATH)
...
def orbit10_data() -> tuple[
    ...
    assert signature == (1, 1, 1, 2, 0, 0, 1)
    assert rows == (
        frozenset((0, 1, 2)),
        frozenset((1, 2, 3)),
        frozenset((2, 4, 5)),
...
def independent_pair_catalogue() -> tuple[dict[str, object], ...]:
    ...
    signature, rows, selected_pair = orbit10_data()
    ...
    assert len(representatives) == 47
    return tuple(
        {
            "case": f"orbit10_pair_{case_index:03d}",
...
def audit_support_and_cuts() -> dict[str, object]:
    ...
    assert len(cuts) == 4_124
    assert len(encoded) == 526
    assert encoded_sizes == {6: 497, 7: 29}
    return {
        "signature": signature,
...
def audit_classifier_action() -> dict[str, object]:
    ...
    assert result["rows"] == ((0, 1, 2), (1, 2, 3), (2, 4, 5))
    assert result["pair"] == (0, 1)
    assert result["row_symmetries"] == 2
    assert result["disjoint_matching_pairs"] == 570_780
    assert result["canonical_pair_orbits"] == 47
    assert result["generator_orbits"] == 47
...
def audit_case(
    ...
    assert result["canonical_pair_types"] == 47
    assert result["positive_capacity_cuts"] == 4_124
    assert result["encoded_capacity_cuts"] == 526
    assert result["encoded_cut_size_inventory"] == {
        "6": 497,
        "7": 29,
    }
    assert result["variables"] == 68_531
...
def load_results(paths: tuple[Path, ...]) -> tuple[dict[str, object], ...]:
    ...
    assert len({result["case"] for result in results}) == 47
...
def main() -> None:
    ...
    assert learned_clauses == 6_073_966
    audit = {
        "status": "pass",
