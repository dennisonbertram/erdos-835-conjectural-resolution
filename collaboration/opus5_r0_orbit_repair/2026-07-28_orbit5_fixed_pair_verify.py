#!/usr/bin/env python3
"""Independent semantic audit for the orbit-5 fixed-pair certificates."""
...
DEFAULT_RESULTS = HERE / "2026-07-28_fixed_pair_orbit5_results.jsonl"
DEFAULT_CNF_DIR = HERE / "2026-07-28_fixed_pair_orbit5_work"
...
SEARCH = load_module("orbit5_verify_search", SEARCH_PATH)
CLASSIFIER = load_module("orbit5_verify_classifier", CLASSIFIER_PATH)
...
def independent_pair_catalogue() -> tuple[dict[str, object], ...]:
    """Reconstruct all representatives without importing the fallback driver."""
    signature, rows, selected_pair = orbit5_data()
    ...
    return tuple(
        {
            "case": f"orbit5_pair_{case_index:03d}",
...
def build_static_case(case: dict[str, object]) -> dict[str, object]:
...
def audit_support_and_cuts() -> dict[str, object]:
    ...
    assert degree_lower == (3, 3, 3, 2, 2, 2, 1, 1, 1, 1, 1, 1, 1)
    assert len(cuts) == 4_040
    assert len(encoded) == 435
    assert encoded_sizes == {6: 413, 7: 22}
    return {
        "signature": signature,
...
def audit_case(
    ...
    assert result["orbit"] == 5
    assert result["case"] == representative["case"]
    assert result["case_index"] == representative["case_index"]
    assert result["invariant"] == json.loads(json.dumps(representative["invariant"]))
...
def main() -> None:
    ...
    assert tuple(result["case_index"] for result in results) == tuple(range(17))
    ...
    print(json.dumps({
        ...
        "compatible_labelled_pairs": 514_080,
        "canonical_pair_types": len(representatives),
        "learned_clauses": sum(case["learned_clauses"] for case in cases),
        "cases": cases,
    }, sort_keys=True))
