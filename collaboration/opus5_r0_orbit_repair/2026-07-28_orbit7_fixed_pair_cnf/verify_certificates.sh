...
replay_dir=$(mktemp -d /private/tmp/orbit7-fixed-replay.XXXXXX)
...
(
    ...
    TMPDIR="$replay_dir" PYTHONPYCACHEPREFIX="$replay_dir/merge-pycache" \
        /usr/bin/python3 orbit7_merge_results.py \
    ...
    cmp orbit7_fixed_pair_results.jsonl "$replay_dir/merged.jsonl"
)
...
TMPDIR="$replay_dir" PYTHONPYCACHEPREFIX="$replay_dir/pycache" \
    /usr/bin/python3 \
    "$certificate_dir/orbit7_fixed_pair_verify.py" \
    --results "$certificate_dir/orbit7_fixed_pair_results.jsonl" \
    --cnf-dir "$replay_dir"
