...
replay_dir=$(mktemp -d /private/tmp/orbit9-fixed-replay.XXXXXX)
...
(
    ...
    TMPDIR="$replay_dir" PYTHONPYCACHEPREFIX="$replay_dir/merge-pycache" \
        /usr/bin/python3 orbit9_merge_results.py \
    ...
    cmp orbit9_fixed_pair_results.jsonl "$replay_dir/merged.jsonl"
)
...
TMPDIR="$replay_dir" PYTHONPYCACHEPREFIX="$replay_dir/pycache" \
    /usr/bin/python3 \
    "$certificate_dir/orbit9_fixed_pair_verify.py" \
    --results "$certificate_dir/orbit9_fixed_pair_results.jsonl" \
    --cnf-dir "$replay_dir"
