#!/usr/bin/env python3
"""Independent semantic audit for the orbit-11 fixed-pair certificates."""
...
DEFAULT_RESULTS = HERE / "2026-07-28_orbit11_fixed_pair_results.jsonl"
DEFAULT_CNF_DIR = HERE / "2026-07-28_orbit11_fixed_pair_raw"
...
SEARCH = load_module("orbit11_verify_search", SEARCH_PATH)
CLASSIFIER = load_module("orbit11_verify_classifier", CLASSIFIER_PATH)
...
def independent_pair_catalogue() -> tuple[dict[str, object], ...]:
    ...
    signature, rows, selected_pair = orbit11_data()
    ...
    return tuple(
        {
            "case": f"orbit11_pair_{case_index:03d}",
...
def audit_support_and_cuts() -> dict[str, object]:
    ...
    assert len(cuts) == 4_325
    assert len(encoded) == 538
    assert encoded_sizes == {6: 533, 7: 5}
    return {
        "signature": signature,
...
def audit_classifier_action() -> dict[str, object]:
    ...
    assert result["canonical_pair_orbits"] == 93
    assert result["generator_orbits"] == 93
    assert result["action_generators"] == 8
    return result
...
def audit_case(
    ...
    assert result["signature"] == [1, 1, 2, 3, 0, 0, 0]
    assert result["rows"] == [[0, 1, 2], [1, 2, 3], [4, 5, 6]]
    assert result["selected_pair"] == [0, 1]
    assert result["third_family"] == 2
    assert result["pair_intersection"] == 2
    assert result["canonical_pair_types"] == 93
    assert result["compatible_labelled_pairs"] == 570_780
    assert result["positive_capacity_cuts"] == 4_325
    assert result["encoded_capacity_cuts"] == 538
    assert result["encoded_cut_size_inventory"] == {"6": 533, "7": 5}
    assert result["variables"] == 67_594
...
def main() -> None:
    ...
    assert learned_clauses == 11_441_006
    print(json.dumps({
        "status": "pass",
