#!/usr/bin/env python3
"""Independent semantic audit for the orbit-8 fixed-pair certificates.
...
DEFAULT_RESULTS = HERE / "2026-07-28_orbit8_fixed_pair_results.jsonl"
DEFAULT_CNF_DIR = HERE / "2026-07-28_orbit8_fixed_pair_raw"
...
SEARCH = load_module("orbit8_verify_search", SEARCH_PATH)
CLASSIFIER = load_module("orbit8_verify_classifier", CLASSIFIER_PATH)
...
def independent_pair_catalogue() -> tuple[dict[str, object], ...]:
    ...
    signature, rows, selected_pair = orbit8_data()
    ...
    return tuple(
        {
            "case": f"orbit8_pair_{case_index:03d}",
...
def audit_support_and_cuts() -> dict[str, object]:
    ...
    assert len(cuts) == 3_872
    assert len(encoded) == 499
    assert encoded_sizes == {6: 406, 7: 92, 8: 1}
    return {
        "signature": signature,
...
def audit_classifier_action() -> dict[str, object]:
    ...
    assert result["rows"] == ((0, 1, 2), (1, 2, 3), (1, 2, 4))
    assert result["pair"] == (0, 1)
    assert result["row_symmetries"] == 2
    assert result["disjoint_matching_pairs"] == 570_780
    assert result["canonical_pair_orbits"] == 16
    assert result["generator_orbits"] == 16
...
def audit_case(
    ...
    assert result["canonical_pair_types"] == 16
    assert result["positive_capacity_cuts"] == 3_872
    assert result["encoded_capacity_cuts"] == 499
    assert result["encoded_cut_size_inventory"] == {
        "6": 406,
        "7": 92,
        "8": 1,
    }
    assert result["variables"] == 71_757
...
def load_results(paths: tuple[Path, ...]) -> tuple[dict[str, object], ...]:
    ...
    assert len({result["case"] for result in results}) == 16
...
def main() -> None:
    ...
    assert learned_clauses == 1_502_474
    audit = {
        "status": "pass",
