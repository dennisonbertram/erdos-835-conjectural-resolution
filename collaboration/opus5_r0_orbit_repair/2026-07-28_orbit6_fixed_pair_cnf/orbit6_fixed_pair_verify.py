#!/usr/bin/env python3
"""Independent semantic audit for the orbit-6 fixed-pair certificates."""
...
DEFAULT_RESULTS = HERE / "2026-07-28_orbit6_fixed_pair_results.jsonl"
DEFAULT_CNF_DIR = HERE / "2026-07-28_orbit6_fixed_pair_raw"
...
SEARCH = load_module("orbit6_verify_search", SEARCH_PATH)
CLASSIFIER = load_module("orbit6_verify_classifier", CLASSIFIER_PATH)
...
def independent_pair_catalogue() -> tuple[dict[str, object], ...]:
    ...
    signature, rows, selected_pair = orbit6_data()
    ...
    assert len(representatives) == 23
    return tuple(
        {
            "case": f"orbit6_pair_{case_index:03d}",
...
def audit_support_and_cuts() -> dict[str, object]:
    ...
    assert len(cuts) == 3_984
    assert len(encoded) == 527
    assert encoded_sizes == {6: 462, 7: 64, 8: 1}
    return {
        "signature": signature,
...
def audit_classifier_action() -> dict[str, object]:
    ...
    assert result["canonical_pair_orbits"] == 23
    assert result["generator_orbits"] == 23
...
def main() -> None:
    ...
    assert learned_clauses == 2_604_987
    print(json.dumps({
        "status": "pass",
