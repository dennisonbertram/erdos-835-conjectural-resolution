...
raw_dir=${RAW_DIR:-"$script_dir/2026-07-28_orbit8_fixed_pair_raw"}
proof_dir=${PROOF_DIR:-"$script_dir/2026-07-28_orbit8_fixed_pair_proof_work"}
package_dir=${PACKAGE_DIR:-"$script_dir/2026-07-28_orbit8_fixed_pair_cnf"}
all_cases="000 001 002 003 004 005 006 007 008 009 010 011 012 013 014 015"
...
cp "$script_dir/2026-07-28_orbit8_fixed_pair_verify.py" \
    "$package_dir/orbit8_fixed_pair_verify.py"
...
cp "$script_dir/2026-07-28_orbit8_fixed_pair_results.jsonl" \
    "$package_dir/orbit8_fixed_pair_results.jsonl"
cp "$script_dir/2026-07-28_orbit8_fixed_pair_classifier_audit.json" \
    "$package_dir/CLASSIFIER_AUDIT.json"
cp "$script_dir/2026-07-28_orbit8_fixed_pair_raw_audit.json" \
    "$package_dir/RAW_AUDIT.json"
cp "$script_dir/2026-07-28_orbit8_certificate_results.json" \
    "$package_dir/CERTIFICATE_RESULTS.json"
cp "$script_dir/2026-07-28_orbit8_toolchain.json" \
    "$package_dir/TOOLCHAIN.json"
cp "$script_dir/2026-07-28_orbit8_verify_certificates.sh" \
    "$package_dir/verify_certificates.sh"
cp "$script_dir/2026-07-28_orbit8_generate_proofs.sh" \
    "$package_dir/generate_proofs.sh"
cp "$script_dir/2026-07-28_orbit8_replay_raw.sh" \
    "$package_dir/replay_raw.sh"
cp "$script_dir/2026-07-28_orbit8_package_certificates.sh" \
    "$package_dir/package_certificates.sh"
cp "$script_dir/2026-07-28_orbit8_cut_sufficiency.md" \
...
for case_index in $all_cases; do
    case_name=orbit8_pair_$case_index
    record_and_compress \
...
(
    ...
    shasum -a 256 \
        ...
        TOOLCHAIN.json \
        generate_proofs.sh \
        orbit8_fixed_pair_results.jsonl \
        orbit8_fixed_pair_verify.py \
