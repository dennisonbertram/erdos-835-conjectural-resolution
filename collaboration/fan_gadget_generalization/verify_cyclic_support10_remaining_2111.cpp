...
int main(int argc,char**argv){
 ...
 for(int q=0;q<228;++q)for(int p:g[q])for(int m:g[q])if(p!=m){S v=vec(tc,{p},{m});uint64_t hv=hashv(v);int k=moves.size();moves.push_back({q,v,hv});byhash[hv].push_back(k);low[hv&((1<<24)-1)]=1;for(auto [r,x]:v)idx[r][x>0].push_back(k);}
 uint64_t configs=0,first_hits=0,probes=0,low_hits=0,hash_hits=0,exact_checks=0;S r1,r2;
 for(int q=qb;q<qe;++q){const auto&h=g[q];for(int a=0;a<13;++a)for(int b=a+1;b<13;++b)for(int c=0;c<13;++c){if(c==a||c==b)continue;for(int d=c+1;d<13;++d){if(d==a||d==b)continue;++configs;S target=neg(vec(tc,{h[a],h[b]},{h[c],h[d]}));auto [row1,sign1]=choose(target,idx);for(int i:idx[row1][sign1]){const M&m1=moves[i];if(m1.q==q||!sub(target,m1.v,2,r1))continue;++first_hits;if(r1.empty())continue;uint64_t h1=hashv(r1);auto [row2,sign2]=choose(r1,idx);for(int j:idx[row2][sign2]){const M&m2=moves[j];if(m2.q==q||m2.q==m1.q)continue;++probes;uint64_t hr=h1-m2.h;if(!low[hr&((1<<24)-1)])continue;++low_hits;auto f=byhash.find(hr);if(f==byhash.end())continue;++hash_hits;if(!sub(r1,m2.v,1,r2))continue;for(int k:f->second){++exact_checks;const M&m3=moves[k];if(m3.v!=r2||m3.q==q||m3.q==m1.q||m3.q==m2.q)continue;std::cout<<"FOUND\n";return 0;}}}}}}
