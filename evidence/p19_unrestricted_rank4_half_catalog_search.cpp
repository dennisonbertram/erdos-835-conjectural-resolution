...
// Heuristic search over all 396 isomorphism classes of one-factorizations of
// K_10.  For a fixed factor-to-magnitude bijection and a fixed four-vertex
// anchor, the remaining edge signs are solved exactly by a small CSP.
//
...
//
// A found witness is rigorous and is verified before printing.  Failure or
// timeout is heuristic only because factor magnitude permutations and anchors
// are sampled.
...
static bool solve_outside_csp(
    ...
    for (int state = 0; state < STATES; ++state) {
        ...
        for (int earlier = 0; earlier < depth; ++earlier) {
            const int predicted = mod(-dot_product(
                x[earlier][assignment[earlier]],
                transformed[depth][state]
            ));
            if (sign_class(predicted) != required[earlier][depth]) {
                compatible = false;
                break;
            }
        }
        if (compatible) {
            ...
            if (solve_outside_csp(
                    depth + 1, x, transformed, required, assignment
                )) {
                return true;
            }
        }
...
static bool test_anchor(
    ...
    for (int anchor_pattern = 0; anchor_pattern < 8; ++anchor_pattern) {
        ...
        std::array<int, 6> assignment{};
        if (!solve_outside_csp(
                0, x, transformed, required, assignment
            )) {
            continue;
        }
